...
to analyze. We'll start by calling the function `get_brewery(state = "ny")` to get brewery data
...
latitude and longitude values across brewery types.
```{r}
nyDist <- ny %>% 
  select(name, brewery_type, latitude, longitude) %>%
  group_by(brewery_type) %>%
  mutate(Mean_Lat = mean(as.numeric(latitude), na.rm = TRUE), 
         Mean_Long = mean(as.numeric(longitude), na.rm = TRUE))
```
