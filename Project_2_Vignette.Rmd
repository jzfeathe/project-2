...
This document is a vignette to explain how to get data from the Open Brewery API using
calls and modifications.
...
Here, we created the function needed to pull a list of breweries from main Open
Brewery API. The "List Breweries" endpoint was the only one we found to be particularly
useful, especially if you're interested in modifications. As such, we felt it 
necessary to create a single function with a section for each modification that can
be used in any desired combination to get to the endpoint.
...
with this data. As the above histogram, the x-axis is the distance from the mean and 
