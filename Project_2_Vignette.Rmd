...
Replace spaces with underscores. Used in name and city functions.
...
```{r}
get_brewery <- function(city = NULL, state = NULL, usa = TRUE, type = NULL, name = NULL,
                        postal = NULL, latitude = NULL, longitude = NULL){
  url <- "https://api.openbrewerydb.org/breweries?per_page=50"
  if (is.null(c(city, state, type, name, postal, latitude, longitude))){
    url <- "https://api.openbrewerydb.org/breweries?per_page=50"
  }
  ...
  if (!is.null(city)){
    ...
    url <- paste0(url, "&by_city=", conv_und(city))
  }
  ...
  if (!is.null(type)){
    ...
    url <- paste0(url, "&by_type=", conv_und(type))
  }
  ...
  if (!is.null(postal)){
    ...
    if (nchar(postal) == 9){
      newpostal <- sub("(.{5})(.*)", "\\1-\\2", postal)
      url <- paste0(url, "&by_postal=", newpostal)
    }
    if (nchar(postal) == 5){
